...
static unsigned int start_sec;
static unsigned int start_usec;
static unsigned int start_msec;
#endif
...
#ifdef _WIN32_WCE /* Windows CE */
	SYSTEMTIME sys_time, epoch_time;
	FILETIME sys_buffer, epoch_buffer, time_buffer;
	ULONGLONG *sys_int, *epoch_int, *time_int, tmp;

	epoch_time.wYear  = 1970;
	epoch_time.wMonth = 1;
	epoch_time.wDay   = 1;
	epoch_time.wHour  = 0;
	epoch_time.wMinute = 0;
	epoch_time.wSecond = 0;
	epoch_time.wMilliseconds = 0;
	epoch_time.wYear = 1970;
	SystemTimeToFileTime(&epoch_time, &epoch_buffer);

	GetSystemTime(&sys_time);
	SystemTimeToFileTime(&sys_time, &sys_buffer);

	sys_int   = (ULONGLONG*)(&sys_buffer);
	epoch_int = (ULONGLONG*)(&epoch_buffer);
	time_int  = (ULONGLONG*)(&time_buffer);

	*time_int = *sys_int - *epoch_int;

	start_sec  = (int)(*time_int/10000000);
	tmp = start_sec;
	start_usec = (int)(*time_int - tmp*10000000);
	start_usec = start_usec/10;
...
#else   /* Windows CE */
	unsigned int now_msec;
	unsigned int diff_msec;

	now_msec = GetTickCount();
	if(now_msec > start_msec) {
		diff_msec = now_msec - start_msec;
	}
	else {
		diff_msec = 0xffffffff - start_msec;
		diff_msec += now_msec;
	}

	Now.sec = start_sec + diff_msec/1000;
	Now.usec= start_usec + (diff_msec%1000)*1000;
#endif  /* Cygwin */
#endif  /* Windows CE */
#endif	/* ARCH_PC_WIN95 */
#if 0
...
#else
			Alarm( EVENTS, "E_delay: select delay returned error\n");
#endif
        }
